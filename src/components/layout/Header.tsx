...
export default function Header() {
  ...
  return <header className="luxury-header">
    ...
    <nav className="category-ribbon" aria-label="Categorii de produse">
      <Link href="/categorii" className="all-categories"><span aria-hidden="true">☷</span> Toate categoriile</Link>
      {shortcuts.map(([name, path]) => <Link key={path} href={`/shop?categorie=${encodeURIComponent(path)}`}>{name}</Link>)}
    </nav>
  </header>;
}
